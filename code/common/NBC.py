""" 
    Function:   Conformer - QCS
    Refs:   Multichannel Speech Separation with Narrow-band Conformer, INTERSPEECH 2022
...
class RelativePositionalMultiHeadAttention(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: Optional[torch.Tensor] = None, value: Optional[torch.Tensor] = None, attn_mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        if key is None:
            key = query
        if value is None:
            value = query
    ...
    def _get_relative_pos_index(self, T: int, device: torch.device) -> torch.Tensor:
        with torch.no_grad():
...
class NBCBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, att_mask: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        r"""
    ...
    def _sa_block(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
