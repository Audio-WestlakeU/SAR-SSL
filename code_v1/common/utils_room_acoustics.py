...
def rt60_from_rirs(h, Fs, vis=False):
    ...
    """
    edc = cal_edc(h)
    t60, corr = cal_rt60(edc, Fs, vis=vis)
...
def find_dp_from_rir(rir, th_ratio=0.5, num_largests = 5, dp_time=2.5, fs=16000):
    """ Function: find dient-path index and value from RIR
        Args:   rir - room impulse response (nsample)
                th_ratio - threshold ratio to detect direct path
                num_largests - number of largest peaks to detect direct path
        Returns: dp_index, dp_value
        Note:   th_ratio=0.5, num_largests=5 is suitable for ACE
    """
    ...
    # print("peak index:", largest_peaks)
    # print("peak valule:", largest_peak_values)
    # print("peak index for larger than 0.5 times the maximum value:", filtered_peaks)
    # print("peak value for larger than 0.5 times the maximum value:", filtered_peak_values)
    # print("peak index with smallest index:", dp_index)
    # print("peak value with smallest index:", dp_value)

    dp_st = dp_index - int(dp_time*fs)
    dp_ed = dp_index + int(dp_time*fs)
    whole_range = np.array(range(0, len(rir)))
    dp_range = (whole_range>=dp_st) & (whole_range<=dp_ed) 
    dp_range = dp_range.astype('float')
    dp_rir = rir*dp_range 

    return dp_index, dp_value, dp_rir
