...
from common.utils_room_acoustics import micpair_dist_in_range, rt60_from_rirs, dpRIR_from_RIR, cart2sph, sph2cart
import dataset as at_dataset

parser = argparse.ArgumentParser(description='Generating multi-channel audio signals')
parser.add_argument('--data-id', type=int, nargs='+', default=[0], metavar='Datasets', help='dataset IDs (default: 9)')
parser.add_argument('--data-type', type=int, nargs='+', default=['rir'], metavar='DataType', help='data types (default: rir, noise)')
parser.add_argument('--data-op', type=str, default='save', metavar='DataOp', help='operation for generated data (default: Save RIR)') # ['save', 'read']
args = parser.parse_args()

opts = opt()
dirs = opts.dir()

rir_list = ['DCASE', 'Mesh', 'MIR', 'ACE', 'dEchorate', 'BUTReverb']
mic_dist_range = [0.03, 0.20]

## Real-world RIR dataset generation class
class DCASERIRDataset():
    ...
    def __init__(self, data_dir, fs, mic_dist_range=[0.03, 0.20], nmic_selected=2, save_dir=None):
        ...
        self.mic_pos_tetra = sph2cart(sph)
                        ...
                        for traj_idx in range(n_traj):
                            ...
                            for hei_idx in range(nHeights):
                                ...
                                for pt_idx in range(n_point):
                                    ...
                                    # Direct-path RIR
                                    dp_RIRs = dpRIR_from_RIR(rir=RIRs, dp_time=2.5, fs=self.fs)
                                    RIR = [RIRs, dp_RIRs]
                                    ...
                                    for mic_idx in range(RIRs.shape[1]):
                                        rt60, r = rt60_from_rirs(RIRs[0, mic_idx, :, 0], self.fs)
                                        T60 += [rt60]
                                        R += [r]
                                    T60 = np.mean(T60)
                                    R = np.mean(r)
                                    ...
                                    acoustic_scene = AcousticScene(
                                        ...
                                        fs = self.fs,
                                        RIR = RIR, # [RIRs, dp_RIRs] (npoints,nch,nsamples,nsources)
                                        )
                                    ...
                                    if self.save_dir is not None:
                                        ...
                                        acous_path =  save_dir + file_name + '.npz'
                    ...
                    for dist in self.dist_set:
                        ...
                        for mic_idxes in mic_combins:   
                            if micpair_dist_in_range(mic_poss[mic_idxes, :], self.mic_dist_range):
                                ...
                                R = np.mean(r)
                                if (abs(R)<0.98):
                                    print('Correlation coefficient (<0.98):', R, 'T60 in dataset:', t60, 'T60 calculated:', T60)

                                # Invalied source postion
                                traj_pts = src_poss[np.newaxis, :, np.newaxis] 
        ...
        for src_idx in range(self.src_poss.shape[0]):
            ...
            for mic_idxes in mic_combins:  
                if micpair_dist_in_range(self.mic_poss[mic_idxes, :], self.mic_dist_range):
                    ...
                    # R = np.mean(r)
                    # if (abs(R)<0.98):
                    # 	print('Correlation coefficient (<0.98):', R, 'T60 in dataset:', self.T60, 'T60 calculated:', T60)
                    T60 = self.T60
                ...
                for array_name in self.array_names:
                    ...
                    for mic_idxes in mic_combins:
                        if micpair_dist_in_range(mic_poss[mic_idxes, :], self.mic_dist_range): 
                            ...
                            # Absorption
                            # absb = self.set_abs(room_env, absb=0.2, refl=0.8)
                            # T60_s = rt60_with_sabine(room_sizes=room_sz, absorption=absb)

                            # T60
                            T60 = self.T60s[room_idx]
                            # T60 = []
                            # R = []
                            # for mic_idx in range(RIRs.shape[1]):
                            #     rt60, r = rt60_from_rirs(RIRs[0, mic_idx, :, 0], self.fs)
                            ...
                            # R = np.mean(r)
                            # if (abs(R)<0.98):
                            #     print('Correlation coefficient (<0.98):', R, 'T60 calculated:', T60)
                ...
                for spk in speaker_names:
                    ...
                    for mic_idxes in mic_combins:   
                        if micpair_dist_in_range(mic_poss[mic_idxes, :], self.mic_dist_range):
                            ...
                            R = np.mean(r)
                            if (abs(R)<0.98):
                                print('Correlation coefficient (<0.98):', R, 'T60 calculated:', T60, 'T60 in dataset:', T60_gt)
...
class ACERIRDataset():
    ...
    def find_dp_from_rir(self, rir, th_ratio=0.5, num_largests = 5):
        ...
        # print("峰值索引:", largest_peaks)
        # print("峰值高度:", largest_peak_values)
        # print("大于最大峰值0.5倍的峰值索引:", filtered_peaks)
        # print("大于最大峰值0.5倍的峰值高度:", filtered_peak_values)
        # print("索引最小的峰值索引:", dp_index)
        # print("索引最小的峰值高度:", dp_value)
            ...
            for array_name in self.array_names:
                for array_pos_name in self.array_pos_names:
                    ...
                    for mic_idx in range(nmic):
                        ...
                        dp_idx, dp_value = self.find_dp_from_rir(rirs[0:nsample, mic_idx])
                ...
                for array_pos_name in self.array_pos_names:
                    ...
                    for mic_idxes in mic_combins:
                        if micpair_dist_in_range(self.mic_poss[array_name][mic_idxes, :], self.mic_dist_range):
                            ...
                            # Source postion & room size
                            traj_pts = np.zeros((1, 3, 1)) # invalid position
                            ...
                            acoustic_scene.DRRfromDataset = DRR_gt
                            rir_num += 1
                            if self.save_dir is not None:
                                save_dir = self.save_dir + '/' + room_name+'/'+array_name+'/'
                                exist_temp = os.path.exists(save_dir)
    ...
    if (args.data_op == 'save'):
        for list_idx in idx_list:
            for data in data_type:
                ...
                save_dirs = dirs['rir']
                print('Dataset=', rir, 'Data=',data)
                save_dir = None
                if data == 'rir':
                    for sdir in save_dirs:
                        if  (rir in sdir) & ('noise' not in sdir):
                            save_dir = copy.deepcopy(sdir)
                            break  
                elif data == 'noise':
                    for sdir in save_dirs:
                        if (rir in sdir) & ('noise' in sdir):
                            save_dir = copy.deepcopy(sdir)
                            break 
                save_dir = save_dir  
                exist_temp = os.path.exists(save_dir)
                ...
                elif (rir == 'dEchorate') | (rir == 'ACE') | (rir == 'BUTReverb') | (rir == 'DCASE') | (rir == 'Mesh') | (rir == 'MIR'):
